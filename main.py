...
def process_video(file_path, speed, start_x, end_x, start_y, end_y, start_frame, end_frame, step, slit_scan_type, output_format) -> list:
    ...
    if v == 0 or ((1 / abs(v)) >= end_frame - start_frame):
        ...
        if slit_scan_type == "Vertical":
            ...
            # angle = math.atan(1 / ((image_width / 2) ** 2 + (image_height / 2) ** 2) ** 0.5)
            ...
            if f:
                ...
                if slit_scan_type == "Radial":
                    for l in range(round((((image_width / 2) ** 2) + ((image_height / 2) ** 2)) ** 0.5)):
                        x = round(image_width / 2) + round(l * math.cos(angle))
                        y = round(image_height / 2) - round(l * math.sin(angle))
                        if y >= 0 and y < image_height and x >= 0 and x < image_width:
                            image[y, x, :] = frame[senter[1] - round(l * math.sin(angle)), senter[0] + round(l * math.cos(angle)), :]
                    angle += angle_step
                    if angle >= 2 * math.pi:
                        break
                elif slit_scan_type == "Circular":
                    # print(f"r: {r}, ri: {ri}, angle: {angle}, percent: {percent}")
                    for a in range(round((2 * math.pi) / angle)):
                        x = round(image_width / 2) + round(r * math.cos(angle * a))
                ...
                if slit_scan_type == "Vertical":
                    ...
                    if v < 0:
                        r = ((image_width / 2) ** 2 + (image_height / 2) ** 2) ** 0.5 - ri * abs(v)
                        percent = round((1 - (r / (((image_width / 2) ** 2 + (image_height / 2) ** 2) ** 0.5))), 4)
                    else:
                        r = v * ri
                    ...
                    if v < 0:
                        ...
                        if r > ((image_width / 2) ** 2 + (image_height / 2) ** 2) ** 0.5:
                            break
                elif slit_scan_type == "Radial":
                    if angle >= 2 * math.pi:
                        break
                    percent = round(angle / (2 * math.pi), 4)
